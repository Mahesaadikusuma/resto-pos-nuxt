...
export const useDeletePermission = async () => {
    ...
    async function handleDelete(id: number, name: string) {
        const instance = modal.open({
            ...
            description: `Apakah Anda yakin ingin menghapus permission "${name}"? Tindakan ini tidak dapat dibatalkan.`
        })
        ...
        try {
            ...
            if (errorMessage.includes('1451') || errorMessage.includes('Integrity constraint violation')) {
                errorMessage = 'Permission tidak bisa dihapus karena masih digunakan oleh role. Silakan hapus atau pindahkan permission tersebut terlebih dahulu.';
            }
            toast.add({
                title: 'Gagal',
